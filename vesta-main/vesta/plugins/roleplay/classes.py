...
from vesta.framework.tools.utilities import ordinal
...
class NekoAPISend:
    ...
    Attributes:
        bot (Vesta): The main bot instance.
    Methods:
    ...
    async def send(
        self, ctx: Context, member: Optional[Member], category: str
    ) -> Message:
        """
            ...
            async with ClientSession(connector=connector) as session:
                async with session.get(
                    ...
                    try:
                        ...
                        return await ctx.embed(
                            "Something went wrong, please try again later!", "warned"
                        )
