...
#                 """
#                 DELETE FROM logging_history
#                 WHERE created_at < NOW() - INTERVAL '7 days'
#                 """
#             )
#             await self.bot.pool.execute("VACUUM logging_history;")
...
#         """
#         Queue a log entry instead of writing directly to database.
#         """
#         try:
...
#             return await ctx.embed(f"Now logging all events in {channel.mention}", "approved")
...
#         embed = Embed(title="Available Logging Events")
...
#         )
#         if record == "DELETE 0":
#             return await ctx.embed(f"Logging in {channel.mention} doesn't exist!", "warned")

#         return await ctx.embed(f"No longer logging in {channel.mention}", "approved")
...
#         )
#         if not record:
#             return await ctx.embed(f"{target.mention} isn't ignored!", "warned")

#         await self.bot.pool.execute(
#             """
#             UPDATE settings
#             SET log_ignore_ids = array_remove(log_ignore_ids, $1)
#             WHERE guild_id = $2
#             """,
#             target.id,
#             ctx.guild.id,
#         )
#         return await ctx.embed(f"No longer ignoring {target.mention} from logging!", "approved")
...
#                 await self.bot.pool.execute(query, *processed_args)
#         except Exception as e:
