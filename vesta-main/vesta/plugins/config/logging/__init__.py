...
from vesta.framework.tools.formatter import plural, human_join, shorten

logger = getLogger("vesta/logging")
...
queued_messages: dict[
    TextChannel | Thread, list[tuple[Embed, Optional[Sequence[File]]]]
] = defaultdict(list)
queue_lock = Lock()
...
async def get_webhook_id(bot: Vesta, channel_id: int) -> Optional[int]:
    """
...
async def get_or_create_webhook(bot: Vesta, channel: TextChannel | Thread) -> Webhook:
    """
...
async def is_ignored(bot: Vesta, guild_id: int, target_id: int) -> bool:
    """
...
async def log(
    ...
    *,
    ...
    """
    bot = cast(Vesta, guild._state._get_client())
    if not guild.me:
        return
    ...
    if files:
        with suppress(HTTPException):
            ...
            return await webhook.send(embed=embed, files=files, silent=True)

        return

    async with queue_lock:
        queued_messages[channel].append(
            (embed, files if files is not MISSING else None)
        )
...
class Logging(Cog):
    ...
    @logs.command(name="add", aliases=("set", "enable"), invoke_without_command=True)
    @has_permissions(manage_guild=True)
    async def logs_add(
        self,
        ctx: Context,
        ...
        *event_names: str,
        ...
        if "all" in event_names:
            ...
            for event_name in event_names:
                try:
                    if event_name.upper() in LogType.__members__:
        ...
        if "all" in event_names:
            event_text = ", ".join(f"`{event}`" for event in valid_events)
    ...
    async def logs_list(self, ctx: Context) -> Message:
        ...
        if not records:
            return await ctx.embed(
                "No **logging channels** have been set up", message_type="warned"
            )
        ...
        for record in records:
            ...
            events = [LogType.from_str(event) for event in record["events"]]
    ...
    @has_permissions(manage_guild=True)
    async def logs_ignore_list(self, ctx: Context) -> Message:
        """
        ...
        entries = [f"<@{record['target_id']}>" for record in records]
    ...
    async def logs_remove(
        ...
        *event_names: str,
        ...
        if not record:
            return await ctx.embed(
                f"No **events** are set up for {channel.mention}", message_type="warned"
            )
        ...
        if len(removed_events) == 1:
            ...
            event_text = ", ".join(f"`{event}`" for event in removed_events)
    ...
    async def log_member_update(self, before: Member, after: Member) -> None:
        ...
        if before.guild_banner != after.guild_banner:
            changes.append(("Server Banner", "Server banner changed"))
    ...
    async def log_message_edit(self, before: Message, after: Message) -> None:
        ...
        embed = Embed()
        embed.set_author(name="Message Edited", icon_url=before.author.display_avatar)
        embed.description = ""
    ...
    async def log_bulk_message_delete(self, messages: list[Message]) -> None:
        ...
        embed = Embed()
        embed.set_author(name="Bulk Message Delete", icon_url=None)
        ...
        with suppress(HTTPException, Exception) as e:
            webhook = await get_or_create_webhook(self.bot, logging_channel)
            if not webhook:
                return
    ...
    async def log_role_update(self, entry: AuditLogEntry) -> None:
        ...
        embed.set_author(name="Role Updated", icon_url=entry.user.display_avatar)
    ...
    async def log_member_role_update(self, entry: AuditLogEntry) -> None:
        ...
        embed = Embed()
        embed.set_author(name="Member Roles Updated", icon_url=member.display_avatar)

        moderator_info = "Unknown"
        if entry.user and entry.user != member:
            moderator_info = f"{entry.user.mention} (`{entry.user.id}`)"
        ...
        if removed:
            embed.description = (
                f"{member.mention} was removed from {', '.join(removed)}"
            )

        embed.add_field(name="Moderator", value=moderator_info, inline=False)
    ...
    async def log_channel_creation(self, entry: AuditLogEntry) -> None:
        ...
        embed.set_author(name="Channel Created", icon_url=entry.user.display_avatar)
        embed.set_footer(text=f"User ID: {entry.user.id}")
    ...
    async def log_invite_creation(self, entry: AuditLogEntry) -> None:
        ...
        if entry.user:
            embed.description += f" by {entry.user.mention}"
        embed.set_author(name="Invite Created", icon_url=entry.user.display_avatar)
    ...
    async def log_invite_deletion(self, entry: AuditLogEntry) -> None:
        ...
        if entry.user:
            embed.description += f" by {entry.user.mention}"
        embed.set_author(name="Invite Deleted", icon_url=entry.user.display_avatar)
    ...
    async def log_emoji_creation(self, entry: AuditLogEntry) -> None:
        ...
        if entry.user:
            embed.description += f" by {entry.user.mention}"
        embed.set_author(name="Emoji Created", icon_url=entry.user.display_avatar)
    ...
    async def log_emoji_update(self, entry: AuditLogEntry) -> None:
        ...
        if entry.user:
            embed.description += f" by {entry.user.mention}"
        embed.set_author(name="Emoji Updated", icon_url=entry.user.display_avatar)
    ...
    async def log_sticker_creation(self, entry: AuditLogEntry) -> None:
        ...
        embed.set_author(name="Sticker Created", icon_url=entry.user.display_avatar)
    ...
    async def log_sticker_update(self, entry: AuditLogEntry) -> None:
        ...
        if entry.user:
            embed.description += f" by {entry.user.mention}"
        embed.set_author(name="Sticker Updated", icon_url=entry.user.display_avatar)
    ...
    async def log_voice_state_update(
        ...
        """
        embed = Embed()
        embed.set_author(name="Voice State Updated", icon_url=member.display_avatar)
        ...
        if before.channel == after.channel and after.channel:
            ...
            embed.add_field(
                name="Current Channel", value=f"{after.channel.mention}", inline=False
            )
            ...
            embed.add_field(
                name="Previous Channel", value=f"{before.channel.mention}", inline=False
            )
            embed.add_field(
                name="Current Channel", value=f"{after.channel.mention}", inline=False
            )
