...
class AntiRaid(Cog):
    ...
    @antiraid.command(
        name="mentions", example="(on/off) (--threshold 5) (--punishment ban)"
    )
    @has_permissions(manage_guild=True)
    ...
    @antiraid.command(
        name="avatar",
        aliases=["pfp"],
        example="(on/off) (--threshold 5) (--punishment ban)",
    )
    @has_permissions(manage_guild=True)
    ...
    async def submit_incident(
        ...
        try:
            properties = {
                ...
                "mitigation_actions": ["invites_disabled", "dms_disabled", "lockdown"],
            ...
            log.warning(
                f"Analytics failed for antiraid incident - Will retry in 5 minutes"
            )
        ...
        with suppress(HTTPException):
            ...
            embed.add_field(
                name="**Action**",
                value=(
                    "New members & DMs have been temporarily restricted for an **hour**"
                ),
                inline=True,
            )
        ...
        try:
            if punishment == "ban":
                ...
                await ModConfig.sendlogs(
                    self.bot, "antiraid_ban", bot_member, member, reason
                )
                return True
                ...
                await ModConfig.sendlogs(
                    self.bot, "antiraid kick", bot_member, member, reason
                )
                return True

            elif punishment == "timeout":
                await member.timeout(duration=timedelta(days=27), reason=reason)
                await ModConfig.sendlogs(
                    self.bot,
                    "antiraid timeout",
                    bot_member,
                ...
                await ModConfig.sendlogs(
                    self.bot, "antiraid strip", bot_member, member, reason
                )
                return True
    ...
    @antiraid_filter.command(
        name="links", example="(invites/external/all) (on/off) (--punishment delete)"
    )
    @has_permissions(manage_guild=True)
        ...
        *,
        ...
        try:
            ...
            if flags.punishment == "timeout":
                actions.append(
                    AutoModRuleAction(
                        type=AutoModRuleActionType.timeout, duration=timedelta(hours=1)
                    )
                )
            ...
            return await ctx.approve(
                f"Created {filter_type} filter with **{flags.punishment}** punishment"
            )
    ...
    @antiraid_filter.group(
        name="exempt", aliases=["whitelist"], invoke_without_command=True
    )
    @has_permissions(manage_guild=True)
            ...
            for rule in rules:
                if rule.name.startswith("Evict -"):
                    if role.id in rule.exempt_roles:
                        await rule.edit(
                            exempt_roles=[r for r in rule.exempt_roles if r != role.id]
                        )
                        updated = True

            if updated:
                return await ctx.approve(
                    f"Removed {role.mention} from filter exemptions"
                )
            else:
    ...
    async def exempt_list(self, ctx: Context) -> Message:
        ...
        try:
            ...
            if not exempt_roles:
                return await ctx.embed("No roles are exempt from filters!", "warned")
            ...
            return await ctx.embed(
                "I need `manage_guild` permissions to manage AutoMod rules", "warned"
            )
        except discord.HTTPException as e:
            return await ctx.embed(f"Failed to fetch exemptions: {e}", "warned")
