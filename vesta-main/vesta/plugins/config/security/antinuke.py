...
from vesta.framework.tools.conversion import Status, Duration
...
class Flags(FlagConverter):
    ...
    punishment: str = flag(
        default="ban", description="The punishment to apply (ban/kick/strip)"
    )
...
class Settings(BaseModel):
    ...
    def is_trusted(self, member: Context | Member) -> bool:
        ...
        return member.id in {self.guild.owner_id, *self.trusted_admins}
    ...
    async def check_threshold(self, bot: Vesta, member: Member, module: str) -> bool:
        """
    ...
    @classmethod
    async def revalidate(cls, bot: Vesta, guild: Guild) -> Optional[Self]:
        """
    ...
    @classmethod
    async def fetch(cls, bot: Vesta, guild: Guild) -> Self:
        """
...
class AntiNuke(Cog):
    ...
    async def antinuke_settings(self, ctx: Context) -> Message:
        ...
        if not config:
            return await ctx.embed(
                "The **antinuke system** hasn't been configured yet!", "warned"
            )
        ...
        for name, module in protection_modules:
            emoji = (
                ctx.config.emojis.antinuke.enable
                if module
                else ctx.config.emojis.antinuke.disable
            )
            embed.add_field(
    ...
    async def antinuke_whitelist_list(self, ctx: Context) -> Message:
        ...
        if not entries:
            return await ctx.embed(
                "No members are exempt from the **antinuke system**!,warned"
            )
    ...
    async def antinuke_trust_list(self, ctx: Context) -> Message:
        ...
        if not entries:
            return await ctx.embed(
                "No members are allowed to manage the **antinuke system**!", "warned"
            )
    ...
    async def antinuke_ban(
        ...
        *,
        ...
        if punishment not in ["ban", "kick", "strip"]:
            return await ctx.embed(
                "Punishment must be `ban`, `kick`, or `strip`!", "warned"
            )
        ...
        if not status:
            return await ctx.embed(
                "No longer preventing members from being banned", "approved"
            )
    ...
    async def antinuke_kick(
        ...
        *,
        ...
        if not status:
            return await ctx.embed(
                "No longer monitoring members from being kicked", "approved"
            )
    ...
    async def antinuke_role(
        ...
        *,
        ...
        if not status:
            return await ctx.embed(
                "No longer monitoring role modifications", "approved"
            )
    ...
    async def antinuke_channel(
        ...
        *,
        ...
        if not status:
            return await ctx.embed(
                "No longer monitoring channel modifications", "approved"
            )
    ...
    async def antinuke_webhook(
        ...
        *,
        ...
        if not status:
            return await ctx.embed(
                "No longer monitoring webhook modifications", "approved"
            )
    ...
    async def antinuke_emoji(
        ...
        *,
        ...
        if not status:
            return await ctx.embed("No longer monitoring emoji modifications!approved")
    ...
    async def antinuke_reset(self, ctx: Context) -> Message:
        ...
        return await ctx.embed(
            "Successfully reset the **antinuke settings**", "approved"
        )

    @Cog.listener("on_audit_log_entry_bot_add")
    async def antinuke_monitor_bot(self, entry: AuditLogEntry):
        """
        Listen for bot addition attempts and ban the perpetrator and the bot.
        """
        start = time()
        guild = entry.guild
    ...
    async def antinuke_monitor_ban(self, entry: AuditLogEntry):
        """
        Listen for ban attempts and punish the perpetrator if they exceed the threshold.
        """
        start = time()
        guild = entry.guild
        perpetrator = entry.user
    ...
    async def antinuke_monitor_kick(self, entry: AuditLogEntry):
        """
        Listen for kick attempts and punish the perpetrator if they exceed the threshold.
        """
        start = time()
        guild = entry.guild
        perpetrator = entry.user
    ...
    async def antinuke_monitor_role(self, entry: AuditLogEntry):
        """
        Listen for role creation, deletion, or modification attempts and punish the perpetrator if they exceed the threshold.
        """
        start = time()
        guild = entry.guild
        perpetrator = entry.user
    ...
    async def antinuke_monitor_channel(self, entry: AuditLogEntry):
        """
        Listen for channel creation, deletion, or modification attempts and punish the perpetrator if they exceed the threshold.
        """
        start = time()
        guild = entry.guild
        perpetrator = entry.user
        ...
        try:
            if config.channel.punishment == "ban":
                await guild.ban(
                    perpetrator, reason=f"Channel {verb[1]} attempt detected"
                )
                ...
                await perpetrator.edit(
                    roles=[], reason=f"Channel {verb[1]} attempt detected"
                )

        except HTTPException as exc:
            error_details = exc.text
            await config.dispatch_log(
                self.bot,
                perpetrator,
                module="channel",
    ...
    async def antinuke_monitor_webhook(self, entry: AuditLogEntry):
        """
        Listen for webhook creation attempts and punish the perpetrator if they exceed the threshold.
        """
        start = time()
        guild = entry.guild
        perpetrator = entry.user
        ...
        try:
            if config.webhook.punishment == "ban":
                ...
                await guild.kick(
                    perpetrator, reason="Webhook creation attempt detected"
                )

            elif config.webhook.punishment == "strip":
                await perpetrator.edit(
                    roles=[], reason="Webhook creation attempt detected"
                )
    ...
    async def antinuke_monitor_emoji(self, entry: AuditLogEntry):
        """
        Listen for emoji creation, deletion, or modification attempts and punish the perpetrator if they exceed the threshold.
        """
        start = time()
        guild = entry.guild
        perpetrator = entry.user
        ...
        try:
            if config.emoji.punishment == "ban":
                ...
                await guild.kick(
                    perpetrator, reason=f"Emoji {verb[1]} attempt detected"
                )

            elif config.emoji.punishment == "strip":
                await perpetrator.edit(
                    roles=[], reason=f"Emoji {verb[1]} attempt detected"
                )
