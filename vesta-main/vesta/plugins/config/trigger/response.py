...
class Response(Cog):
        ...
        *,
        ...
        return await ctx.embed(
            ...
            + (
                ...
                if any(
                    getattr(parsed_flags, flag.attribute)
                    for flag in parsed_flags.values
                )
                else ""
            ),
    ...
    async def response_remove(self, ctx: Context, *, trigger: str) -> Message:
        ...
        return await ctx.embed(
            f"Removed response trigger for **{trigger}**",
            message_type="approved",
        )
    ...
    async def response_clear(self, ctx: Context) -> Message:
        ...
        if result == "DELETE 0":
            return await ctx.embed(
                "No response triggers exist for this server!",
                message_type="warned",)
    ...
    async def response_list(self, ctx: Context) -> Message:
        ...
        return await ctx.paginate(
                entries=triggers,
                embed=Embed(title=f"{len(triggers)} Response Trigger{'s' if len(triggers) > 1 else ''}"
            ),
        )
