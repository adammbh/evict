...
from vesta.framework.tools.formatter import codeblock, plural


class Disboard(Cog):
    """
    Receive a reminder every 2h to bump the server.
    """
    ...
    async def disboard_toggle(self, ctx: Context) -> Message:
        ...
        return await ctx.embed(
            message=f"{'Now receiving' if status else 'No longer receiving'} **bump reminders**",
    ...
    async def disboard_channel(self, ctx: Context, *, channel: TextChannel) -> Message:
        ...
        return await ctx.embed(
            message=f"Now sending **bump reminders** to {channel.mention}",
            message_type="approved",
        )
    ...
    async def disboard_channel_remove(self, ctx: Context) -> Message:
        ...
        return await ctx.embed(
            message="Now sending **bump reminders** to the **last bumped** channel",
            message_type="approved",
        )
    ...
    @has_permissions(manage_channels=True)
    async def disboard_message(self, ctx: Context, *, message: str) -> Message:
        """
        ...
        return await ctx.embed(
            message=f"Successfully set ``{message}`` as the **bump reminder** message",
            message_type="approved",
        )
    ...
    async def disboard_message_remove(self, ctx: Context) -> Message:
        ...
        return await ctx.embed(
            message="Now using the **default** bump reminder message",
            message_type="approved",
        )
    ...
    @has_permissions(manage_channels=True)
    async def disboard_message_thank(self, ctx: Context, *, message: str) -> Message:
        """
        ...
        return await ctx.embed(
            message=f"Successfully set ``{message}`` as the **thank you** message",
            message_type="approved",
        )
    ...
    async def disboard_message_thank_remove(self, ctx: Context) -> Message:
        ...
        return await ctx.embed(
            message="No longer sending a **thank you** message", message_type="approved"
        )
    ...
    async def disboard_settings(self, ctx: Context) -> Message:
        ...
        """
        record: Optional[DisboardRecord] = await self.bot.pool.fetchrow(
            """
            SELECT *
        ...
        if not record:
            return await ctx.embed(
                message="The **bump reminder** system has not been enabled!",
                message_type="warned",
            )
    ...
    async def disboard_leaderboard(self, ctx: Context) -> Message:
        ...
        if not members:
            return await ctx.embed(
                message="No members have bumped the server!", message_type="warned"
            )

        return await ctx.paginate(
            entries=members, embed=Embed(title="Bump Leaderboard")
        )
