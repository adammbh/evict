...
class AutoRoles(Cog):
    ...
    async def autorole_reassign_ignore(
        ...
        *,
        ...
        if role in ctx.settings.reassign_ignore:
            return await ctx.embed(
                f"{role.mention} is already being ignored!",
                message_type="warned"
            )
        ...
        return await ctx.embed(
            f"Now ignoring {role.mention} from being reassigned",
            message_type="approved"
        )
    ...
    async def autorole_reassign_ignore_remove(
        ...
        *,
        ...
        await ctx.settings.update()
        return await ctx.embed(
            f"Now allowing {role.mention} to be reassigned",
            message_type="approved"
        )
    ...
    async def autorole_reassign_ignore_list(self, ctx: Context) -> Message:
        ...
        return await ctx.paginate(entries=entries, embed=Embed(title=f"{len(entries)} Ignored Roles"))
    ...
    async def autorole_add(
        ...
        *,
        ...
        if flags.action == "remove" and not flags.delay:
            return await ctx.embed(
                "You must specify a delay when removing roles!",
                message_type="warned"
            )
    ...
    async def autorole_remove(
        ...
        if result == "DELETE 0":
            return await ctx.embed(
                f"{role.mention} is not an existing auto role!",
                message_type="warned"
            )

        return await ctx.embed(
            f"Successfully removed {role.mention} as an auto role",
            message_type="approved"
        )
    ...
    async def autorole_clear(self, ctx: Context) -> Message:
        ...
        if result == "DELETE 0":
            return await ctx.embed(
                "No auto roles exist for this server!",
                message_type="warned"
            )
    ...
    async def autorole_list(self, ctx: Context) -> Message:
        ...
        return await ctx.paginate(entries=roles, embed=Embed(title=f"{len(roles)} Auto Roles"))
