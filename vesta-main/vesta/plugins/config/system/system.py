...
class System(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def _get_cached_messages(
        ...
        cache_dict[cache_key] = {"timestamp": time.time(), "data": records}
        return records

    async def _process_message_send(
        self, member: Member, message_type: str
    ) -> List[Message]:
        ...
        async def process_single_message(record):
            ...
            channel = cast(
                Optional[TextChannel | Thread], guild.get_channel_or_thread(channel_id)
            )
        ...
        sent_messages = [
            msg for msg in messages if msg and not isinstance(msg, Exception)
        ]
            ...
            try:
                async with self.bot.pool.acquire() as conn:
                    ...
                    if config and config["message"]:
                        script = await self.bot.embed_build.alt_convert(
                            member, config["message"]
                        )
    ...
    async def welcome_delete(self, member: Member):
        ...
        removal, identifiers = await asyncio.gather(
            self.bot.pool.fetchval(
                "SELECT welcome_removal FROM settings WHERE guild_id = $1", guild.id
            ),
