...
from vesta.framework.tools.formatter import plural, shorten

log = getLogger("vesta/star")
...
class StarboardConfig:
    ...
    def __init__(self, *, bot: Vesta, record: StarboardRecord):
        self.bot = bot
...
class Starboard(Cog):
    ...
    @starboard.command(
        name="add", aliases=["create"], example="(#channel) (emoji) (--flag)"
    )
    @has_permissions(manage_guild=True)
        ...
        *,
        ...
        try:
            ...
            return await ctx.embed(
                f"I'm not capable of using **{emoji}**!\n"
        ...
        return await ctx.embed(
            f"Added a starboard for {emoji} in {channel.mention}", 
            message_type="approved"
        )

    @starboard.command(
        name="remove", aliases=["delete", "del", "rm"]
    )
    @has_permissions(manage_guild=True)
    ...
    async def starboard_clear(self, ctx: Context) -> Message:
        ...
        if result == "DELETE 0":
            return await ctx.embed(
                "No starboards exist for this server!",
                message_type="warned"
            )
    ...
    async def starboard_list(self, ctx: Context) -> Message:
        ...
        return await ctx.paginate(
            entries=channels,
            embed=Embed(title=f"{len(channels)} Starboard{'s' if len(channels) != 1 else ''}")
        )
