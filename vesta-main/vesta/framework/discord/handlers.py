...
if TYPE_CHECKING:
    from vesta.framework.discord import format_timespan, human_join, plural


class CommandErrorHandler:
    """
    Custom command error handler for Vesta.
    """
    ...
    async def on_command_error(self, ctx: Context, exc: CommandError) -> Any:
        ...
        if not ctx.guild:
            ...
            can_send = (
                ctx.channel.permissions_for(guild_me).send_messages
                and ctx.channel.permissions_for(guild_me).embed_links
            )
        ...
        if isinstance(
            ...
            if isinstance(exc, TooManyFlags):
                ...
                return await ctx.embed(
                    f"You must specify the **{exc.flag.name}** flag!", "warned"
                )
        ...
        if isinstance(exc, CommandInvokeError):
            return await ctx.embed(exc.original, "warned")
            ...
            return await ctx.embed(
                f"No **member** was found matching **{exc.argument}**!", "warned"
            )

        elif isinstance(exc, UserNotFound):
            return await ctx.embed(
                f"No **user** was found matching `{exc.argument}`!", "warned"
            )

        elif isinstance(exc, RoleNotFound):
            return await ctx.embed(
                f"No **role** was found matching **{exc.argument}**!", "warned"
            )

        elif isinstance(exc, ChannelNotFound):
            return await ctx.embed(
                f"No **channel** was found matching **{exc.argument}**!", "warned"
            )

        elif isinstance(exc, BadInviteArgument):
            return await ctx.embed("Invalid **invite code** provided!", "warned")
            ...
            return await ctx.embed(f"The input must be {label}!", "warned")
            ...
            return await ctx.embed(
                f"You're missing the {permissions} permission{_plural}!", "warned"
            )

        elif isinstance(exc, NSFWChannelRequired):
            return await ctx.embed(
                "This command can only be used in NSFW channels!", "warned"
            )
            ...
            if isinstance(exc, (HTTPException, NotFound)) and not isinstance(
                ...
                if "Unknown Channel" in exc.text:
                    return
                return await ctx.embed(exc.text.capitalize(), "warned")
            ...
            if isinstance(exc, (Forbidden, CommandInvokeError)):
                ...
                if isinstance(error, Forbidden):
                    ...
                    return await ctx.embed(error_msg, f"Error: {str(error)}", "warned")
