...
from discord.ext.commands import MissingPermissions, CommandError, check, Converter
    ...
    async def convert(self, ctx: Context, argument: str):
        ...
        if not argument in valid_permissions:
            return await ctx.embed(
                f"This is **not** a valid permission. Please run `{ctx.clean_prefix}fakepermissions permissions` to check all available permissions!",
                "error",
            )
