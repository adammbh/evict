...
if TYPE_CHECKING:
    from vesta.framework.discord import Context
...
class Script:
    ...
    def __init__(
        ...
        self.blocks = blocks
        self._fixed_template = parse(self.template, self.blocks)
    ...
    def data(self) -> ScriptData:
        ...
        for node in self.nodes:
            if node.name in ("content", "message", "msg"):
                ...
                if first_part in style_map:
                    ...
                    disabled = False
                    if len(parts) > 4:
                        disabled = "disabled" in parts[4].strip()
                else:
                    style_text = "link"
                    label = parts[0].strip()
                    ...
                    disabled = False
                    if len(parts) > 3:
                        disabled = "disabled" in parts[3].strip()

                if not label or not url:
                    continue

                style = style_map.get(style_text, ButtonStyle.link)
                ...
                if node.name == "embed":
                    data["embeds"].append(Embed(color=0x1E1F22))
        ...
        data["embeds"] = [embed for embed in data["embeds"] if embed]
