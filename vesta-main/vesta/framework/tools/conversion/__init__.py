...
from vesta.shared import config
...
class PartialAttachment:
    ...
    @staticmethod
    async def read(ctx: Context, url: URL | str) -> tuple[bytes, str]:
        async with ClientSession() as client:
            async with client.get(url, proxy="http://127.1:40000") as resp:
    ...
    async def fallback(cls, ctx: Context) -> "PartialAttachment":
        ...
        buffer, content_type = await cls.read(ctx, str(attachment_url))
        return cls(
            attachment_url,
            buffer,
