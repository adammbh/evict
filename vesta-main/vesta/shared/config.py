...
from typing import Any, Dict, List, Optional, Self, TYPE_CHECKING, ClassVar, Literal
...
class Defaults(BaseModel):
    prefix: str = ";"
    help_menu: Literal["Evict", "Basic", "Dropdown"] = "Evict"
...
class FunEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_FUN_")
...
class DeviceEmojis(BaseSettings):
    ...
    phone_online: str = "<:phone_online:1348051801248759959>"
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_DEVICES_")
...
class EconomyEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_ECONOMY_")
...
class PollEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_POLL_")
...
class StaffEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_STAFF_")
...
class InterfaceEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_INTERFACE_")
...
class PaginatorEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_PAGINATOR_")
...
class AudioEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_AUDIO_")
...
class AntinukeEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_ANTINUKE_")
...
class BadgesEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_BADGES_")
...
class ContextEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_CONTEXT_")
...
class StatusEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_STATUS_")
...
class SocialEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_SOCIAL_")
...
class MiscEmojis(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="EMOJIS_MISC_")
...
class BunnyCDNBackup(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="BUNNYCDN_BACKUPS_")
...
class BunnyCDNAVH(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="BUNNYCDN_AVH_")
...
class BunnyCDNSocials(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="BUNNYCDN_SOCIALS_")
...
class Proxy(BaseModel):
    protocol: Literal["http", "https", "socks4", "socks5", "socks5h"] = "https"
    host: str = "localhost"
    port: int = 6995
    ...
    @property
    def url(self) -> str:
        return f"{self.protocol}://{f'{quote(self.username)}:{quote(self.password or "")}@' if self.username else ''}{self.host}:{self.port}"
    ...
    def from_url(cls, url: str) -> Self:
        ...
        """
        if (parsed := urlparse(url)) and parsed.scheme not in {
            "http",
            "https",
            "socks4",
            "socks5",
            ...
            raise ValueError(f"Invalid proxy protocol: {parsed.scheme}")
...
class LoggingTypes(BaseModel):
    ...
    guild_exchange: int = 1319466970474610708
    guild_blacklist: int = 1319467116553830460
...
class Lavalink(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="LAVALINK_")
...
class Authentication:
    ...
    proxy: ClassVar[Proxy] = Proxy.from_url(getenv("PROXY_URL", "http://127.1:40000"))
    logging: ClassVar[Logging] = Logging()
    lavalink: ClassVar[Lavalink] = Lavalink()
    owner_ids: ClassVar[List[int]] = loads(
        getenv(
            "OWNER_IDS",
            "[585689685771288600, 270713268085981196, 987183275560820806, 1247076592556183598, 320288667329495040]",
        )
    )
    support_url: ClassVar[str] = getenv("SUPPORT_URL", "https://discord.gg/evict")
...
class Configuration:
    ...
    emojis: Emojis
    defaults: Defaults
    authentication: ClassVar[Authentication] = Authentication()
    ...
    async def from_guild(cls, guild_id: int, bot: "Vesta") -> Self:
    ...
    async def from_user(cls, user_id: int, bot: "Vesta") -> Self:
        config: Dict[str, Any] = loads(
            await bot.pool.fetchval(
                """
                SELECT config 
