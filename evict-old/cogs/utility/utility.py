...
class Utility(Extended, Cog):
    ...
    async def auto_media(self):
        ...
        for record in records:
            try:
                ...
                async with self.bot.session.get(f"{base_url}?token=3nXGd7wMLvHreP") as resp:
                    if resp.status != 200:
                        continue
    ...
    async def avatarhistory(self, ctx: Context, user: Optional[User] = None) -> Message:
        ...
        try:
            ...
            if not image_data:
                return await ctx.warn(f"Could not load any avatar images for {user.mention}!")
    ...
    async def avatarhistory_clear(self, ctx: Context) -> Message:
        ...
        try:
            path = f"/root/cdn/cdn_root/images/avatars/{ctx.author.id}/"
